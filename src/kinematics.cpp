...
static_assert(
    kMaxSwivelSamples == GEOFIK_MAX_N_POINTS, "franky::kMaxSwivelSamples must match the vendored solver's buffer size");
...
// exact by construction, but franka_ik_q4 and franka_ik_q6 fall back internally to the q7
// solver in the wrist (type-2) singularity, and that fallback reaches the pose while
...
// Reconcile franky's end-effector frame with GeoFIK's fixed 'E' frame. Right-multiplying a
// target by this yields the pose GeoFIK expects:
...
// The body of inverseKinematics, with the frame reconciliation and the option validation
// already done by the caller. Touches no shared mutable state (GeoFIK's globals are
// thread_local), so it is safe to run concurrently on distinct targets.
static std::vector<Vector7d> inverseKinematicsPrepared(
    const Affine &O_T_EE, const Affine &ee_to_geofik, double redundancy_value, RedundancyParameter parameter,
    const Affine &F_T_EE, const IKOptions &options) {
  ...
  switch (parameter) {
    case RedundancyParameter::kSwivel:
      ...
      break;
    case RedundancyParameter::kQ7:
      candidate_count = franka_ik_q7(r, roe, redundancy_value, qsols, options.q1_singularity);
      break;
    case RedundancyParameter::kQ6:
      candidate_count = franka_ik_q6(r, roe, redundancy_value, qsols, options.q1_singularity, options.q7_singularity);
      break;
    case RedundancyParameter::kQ4:
      candidate_count = franka_ik_q4(r, roe, redundancy_value, qsols, options.q1_singularity, options.q7_singularity);
      break;
  }
...
std::vector<Vector7d> inverseKinematics(
  ...
  return inverseKinematicsPrepared(O_T_EE, eeToGeofik(F_T_EE), redundancy_value, parameter, F_T_EE, options);
}
...
static std::optional<Vector7d> inverseKinematicsNearestPrepared(
  ...
  const std::vector<Vector7d> solutions =
      inverseKinematicsPrepared(O_T_EE, ee_to_geofik, value, parameter, F_T_EE, options);
