...
void patchMutexRT(std::mutex& mutex) {
  ...
  auto success = pthread_mutex_init(mutex.native_handle(), &attr);
  if (success != 0)
    checkRes(
        pthread_mutex_init(mutex.native_handle(), nullptr),
        "Failed to re-initialize standard mutex after failure to initialize RT mutex.");
  checkRes(success, "Failed to initialize RT mutex.");
